...
export const Purchases = (props) => {
    ...
    useEffect(() => {
        ...
        Firebase.firestore().collection('bookings').where('needToContact', '==', true).orderBy("timestamp", 'desc').limit(count).onSnapshot(snapshot => {
            let allBookings = snapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            }));
        ...
        Firebase.firestore().collection('bookings').where('needToContact', '==', false).orderBy("timestamp", 'desc').limit(count).onSnapshot(snapshot => {
            let allBookings = snapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            }));
        ...
        Firebase.firestore().collection('bookings').where('paid', '==', 'no').orderBy("timestamp", 'desc').limit(count).onSnapshot(snapshot => {
            snapshot.docs.map(doc => {
                setNotpaidBookings(notpaidBookings => [...notpaidBookings, {
                    id: doc.id,
                    ...doc.data()
                }])
                return null;
    ...
    const getMoreUnpaid = () => {
        Firebase.firestore().collection('bookings').where('paid', '==', 'no').orderBy("timestamp", 'desc').startAfter(latestNotpaidBookings).limit(count).onSnapshot(snapshot => {
            if (!snapshot.empty) {
                snapshot.docs.map(doc => {
                    setNotpaidBookings(notpaidBookings => [...notpaidBookings, {
                        id: doc.id,
    ...
    const getMorePaidNtc = () => {
        Firebase.firestore().collection('bookings').where('needToContact', '==', true).orderBy("timestamp", 'desc').startAfter(latestNtcBookings).limit(count).onSnapshot(snapshot => {
            if (!snapshot.empty) {
                let allBookings = snapshot.docs.map(doc => ({
                    id: doc.id,
                    ...doc.data()
    ...
    const getMorePaidNntc = () => {
        Firebase.firestore().collection('bookings').where('needToContact', '==', false).orderBy("timestamp", 'desc').startAfter(latestNntcBookings).limit(count).onSnapshot(snapshot => {
            if (!snapshot.empty) {
                let allBookings = snapshot.docs.map(doc => ({
                    id: doc.id,
                    ...doc.data()
    ...
    return (
        <Styles>
            ...
            <div className="h5 text-center mb-2 mt-4 ml-3 text-secondary">عدد المسجلين :  {allBookingsCount.length}</div>
                ...
                <Tabs defaultActiveKey="required" id="bookings-tabs">
                    <Tab eventKey="required" title={`🔊 جاري (${ntcBookings.length})`}>
                        <Accordion>
                            ...
                            {ntcBookingsLast && (ntcBookings.length <= count) && (
                                <p className="text-center mt-2">
                                    <Button onClick={getMorePaidNtc} variant="link" size="sm">تحميل المزيد</Button>
                                </p>
                            )}
                        </Accordion>
                    </Tab>
                    <Tab eventKey="done" title={`"🎉 تم  (${nntcBookings.length})`}>
                        <Accordion>
                            ...
                            {nntcBookingsLast && (nntcBookings.length <= count) && (
                                <p className="text-center mt-2">
                                    <Button onClick={getMorePaidNntc} variant="link" size="sm">تحميل المزيد</Button>
                                </p>
                            )}
                        </Accordion>
                    </Tab>
                    <Tab eventKey="notpaid" title={`🚫 لم يدفع  (${notpaidBookings.length})`}>
                        <Accordion>
                            ...
                            {notpaidBookingsLast && (notpaidBookings.length <= count) && (
                                <p className="text-center mt-2">
                                    <Button onClick={getMoreUnpaid} variant="link" size="sm">تحميل المزيد</Button>
                                </p>
                            )}
                        </Accordion>
                    </Tab>
